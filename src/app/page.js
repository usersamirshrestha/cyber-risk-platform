...
export default function Home() {
  ...
  useEffect(() => {
    async function initializeSession() {
      try {
        ...
        if (!session) {
          setLoading(false);
          return; // Stop here if no session exists
        }
  ...
  const handleSubmitAudit = async (e) => {
    ...
    try {
      // Create company associated with logged in user_id
      const { data: compData, error: compErr } = await supabase
        ...
        .insert([{ 
          ...
          industry: 'Technology', 
          size: 'Enterprise',
          user_id: user.id // Attach audit to logged-in user!
        }])
        .select()
        .single();
  ...
  // Gated Route UI if not logged in
  if (!user) {
    return (
      <main className="min-h-screen bg-gray-950 text-white flex flex-col items-center justify-center p-6">
        <div className="text-center space-y-4 max-w-md">
          ...
          <a 
            href="/auth" 
            className="inline-block mt-2 px-6 py-3 bg-blue-600 hover:bg-blue-700 text-white font-bold rounded-lg text-xs tracking-wider uppercase transition shadow-lg shadow-blue-900/40"
          >
    ...
    <main className="min-h-screen bg-gray-950 text-white p-6 md:p-12">
      ...
      <div className="max-w-6xl mx-auto flex justify-between items-center mb-8 border-b border-gray-800 pb-4">
        ...
        <button 
          ...
          className="text-xs px-3 py-1.5 bg-gray-900 hover:bg-gray-800 border border-gray-800 rounded-md text-gray-400 font-semibold transition"
        >
          Disconnect Session
        </button>
      </div>
      ...
      <div className="max-w-6xl mx-auto grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          {submitSuccess ? (
            <div className="bg-green-950/30 border border-green-500/40 p-8 rounded-2xl text-center space-y-4">
              ...
              <button 
                ...
                className="mt-4 px-6 py-2 bg-gray-900 border border-gray-700 rounded-lg text-sm text-white"
              >
            ...
            <form onSubmit={handleSubmitAudit} className="space-y-6">
              ...
              <div className="space-y-4">
                {questions.map((q, idx) => (
                  <div key={q.id} className="bg-gray-900 border border-gray-800 rounded-xl p-6 space-y-4">
                    ...
                    <p className="text-gray-200 text-sm font-medium">{q.question_text}</p>
              ...
              <button type="submit" disabled={isSubmitting} className="w-full py-4 bg-blue-600 hover:bg-blue-700 text-white font-bold rounded-xl text-sm tracking-wider uppercase shadow-lg">
        ...
        <div className="lg:col-span-1">
          <div className="sticky top-12 bg-gray-900 border border-gray-800 rounded-xl p-6 text-center space-y-6 shadow-xl">
            <h2 className="text-lg font-bold">Security Rating</h2>
            <div className="py-8 flex items-center justify-center">
              <div className="w-40 h-40 rounded-full border-4 border-gray-800 flex flex-col items-center justify-center bg-gray-950">
